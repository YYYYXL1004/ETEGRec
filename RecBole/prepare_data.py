#!/usr/bin/env python3
"""
数据准备脚本 - 从Amazon原始数据生成RecBole .inter文件和ETEGRec所需的train/valid/test.jsonl
...
def load_and_preprocess(review_file, min_interactions=5):
    ...
    # 使用parent_asin作为item_id
    df['item_id'] = df['parent_asin']
    df['timestamp'] = df.get('timestamp', range(len(df))) / 1000
...
def save_inter_file(df, output_dir, dataset_name):
    ...
    with open(inter_file, 'w', encoding='utf-8') as f:
        ...
        for _, row in df.iterrows():
            f.write(f"{row['user_id']}\t{row['item_id']}\t{row.get('rating', 5.0)}\t{row['timestamp']}\t{row['split']}\n")
...
def main():
    print("=" * 70)
    print("🎵 数据准备 - Amazon Musical Instruments 2023")
    print("=" * 70)
